...
static uint8_t g_awb = 1; //TMS edit? (originally 1)
static uint8_t g_aec = 1; //TMS edit? (originally 1)
...
/// TMS ///
	uint8_t inde = ecv;
	inde--;
	  uint32_t ecv_array [20] = {300, 100, 200, 900, 600, 1100, 500, 730, 510, 1000, 1500, 9000, 8600, 8680, 16711680, 8700, 200000, 2130771775, 600000, 2147483455};
  	uint32_t val = ecv_array[inde];
/// TMS ///
...
//	prm_add("Brightness", 0, 
	...
	// TMS edit: set AEC to 0
	prm_add("AEC Enable", PRM_FLAG_ADVANCED, 
		"@c Camera Enables/disables Auto Exposure Correction, 0=disabled, 1=enabled (default 1)", UINT8(0), END);
...
//	prm_add("AEC Value", PRM_FLAG_HEX_FORMAT | PRM_FLAG_ADVANCED, 
	...
	prm_add("Exposure (1-20)", 0, 
		"Sets the Auto Exposure Correction value.  The parameter only applies when AEC Enable=0. Use the command \"cam_getECV\" to get the current value (default 0)", UINT32(13), END);
																		   
	// TMS edit: set AWB to 0
	prm_add("AWB Enable", PRM_FLAG_ADVANCED, 
		"@c Camera Enables/disables Auto White Balance, 0=disabled, 1=enabled on startup, 2=always enabled (default 1)", UINT8(0), END);
